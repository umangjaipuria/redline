  ...
  if (commandName === "comment") {
    ...
    printJson(
      createComment(documentPath, {
        anchor: anchorForQuote(quote, withOccurrence.value, threadId),
        quote,
        body,
...
function anchorForQuote(
  ...
  const anchor: CommentAnchor = { type: "text-range", quote };
  const occurrence = normalizeOccurrence(occurrenceValue);
  if (occurrence !== undefined) anchor.occurrence = occurrence;
